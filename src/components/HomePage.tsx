...
const HomePage = () => {
  ...
  React.useEffect(() => {
    ...
    const animate = () => {
      ...
      const position = (elapsed * speed / 1000) % 1000; // Reset every 1000px
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="mt-8 md:mt-12">
          ...
          <div className="relative h-32 md:h-40 overflow-hidden rounded-lg">
            <div 
              className="absolute flex space-x-4 transition-transform duration-1000 ease-linear will-change-transform"
              style={{ transform: `translateX(-${carouselPosition}px)` }}
            >
              {[...Array(10)].map((_, i) => (
                <div key={i} className="w-32 h-32 md:w-40 md:h-40 bg-gray-500 flex-shrink-0 rounded-lg"></div>
              ))}
            </div>
          </div>
        </div>
